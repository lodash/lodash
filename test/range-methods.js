...
describe('range methods', function () {
  lodashStable.each(['range', 'rangeRight'], function (methodName) {
    ...
    it(
      '`_.' +
        methodName +
        '` should infer the sign of `step` when only `start` and `end` are given',
      function () {
    ...
    it(
      '`_.' + methodName + '` should work with a `start`, `end`, and `step`',
      function () {
    ...
    it(
      '`_.' + methodName + '` should work with a `step` larger than `end`',
      function () {
    ...
    it(
      '`_.' + methodName + '` should work with a negative `step`',
      function () {
        assert.deepStrictEqual(func(0, -4, -1), resolve([0, -1, -2, -3]))
