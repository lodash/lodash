; (function () {
  ...
  (function () {
    ...
    QUnit.test('should support multiple variables via comma-separated variable option', function (assert) {
      assert.expect(1);

      var compiled = _.template('<%= x %> + <%= y %> = <%= x + y %>', { 'variable': 'x, y' });
      assert.strictEqual(compiled(2, 3), '2 + 3 = 5');
    });

    QUnit.test('should support three or more variables', function (assert) {
      assert.expect(1);

      var compiled = _.template('<%= a %> <%= b %> <%= c %>', { 'variable': 'a, b, c' });
      assert.strictEqual(compiled('Hello', 'World', '!'), 'Hello World !');
    });

    QUnit.test('should support multiple variables with escape delimiter', function (assert) {
      assert.expect(1);

      var compiled = _.template('<%- name %>, age <%= age %>', { 'variable': 'name, age' });
      assert.strictEqual(compiled('<script>', 25), '&lt;script&gt;, age 25');
    });

    QUnit.test('should generate correct function signature for multiple variables', function (assert) {
      assert.expect(1);

      var compiled = _.template('<%= x %> <%= y %>', { 'variable': 'x, y' });
      assert.ok(compiled.source.indexOf('function(x, y)') !== -1);
    });

    QUnit.test('should maintain backward compatibility with single variable', function (assert) {
      assert.expect(1);

      var compiled = _.template('hi <%= data.user %>!', { 'variable': 'data' });
      assert.strictEqual(compiled({ user: 'fred' }), 'hi fred!');
    });

    QUnit.test('should forbid code injection through multiple variables', function (assert) {
      assert.expect(1);

      assert.raises(function () {
        _.template('', { 'variable': 'x, ){console.log(process.env)}; //' });
      });
    });
  }());
