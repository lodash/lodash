...
describe('filter methods', function () {
  lodashStable.each(['filter', 'reject'], function (methodName) {
    ...
    it(
      '`_.' +
        methodName +
        '` should not modify the resulting value from within `predicate`',
      function () {
    ...
    it(
      ...
      function () {
        assert.deepStrictEqual(func(objects, objects[1]), [
          objects[isFilter ? 1 : 0]
        ])
      }
    )
    ...
    it('`_.' + methodName + '` should work in a lazy sequence', function () {
      ...
      var object = lodashStable.zipObject(
        lodashStable.times(LARGE_ARRAY_SIZE, function (index) {
