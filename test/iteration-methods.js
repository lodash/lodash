...
describe('iteration methods', function () {
  ...
  var arrayMethods = ['findIndex', 'findLastIndex', 'maxBy', 'minBy']
  ...
  var forInMethods = ['forIn', 'forInRight', 'omitBy', 'pickBy']
  ...
  lodashStable.each(
    lodashStable.difference(methods, objectMethods),
    function (methodName) {
      var array = [1, 2, 3],
        func = _[methodName],
  ...
  lodashStable.each(
    lodashStable.difference(methods, unwrappedMethods),
    function (methodName) {
      var array = [1, 2, 3],
      ...
      it(
        '`_.' +
          methodName +
          '` should return a wrapped value when implicitly chaining',
        function () {
          if (!(isBaseEach || isNpm)) {
  ...
  lodashStable.each(collectionMethods, function (methodName) {
    ...
    it(
      '`_.' +
        methodName +
        '` should use `isArrayLike` to determine whether a value is array-like',
      function () {
        if (func) {
  ...
  lodashStable.each(
    lodashStable.difference(
      lodashStable.union(methods, collectionMethods),
      arrayMethods
    ),
    function (methodName) {
      var func = _[methodName],
      ...
      it(
        '`_.' + methodName + '` should ignore added `object` properties',
        function () {
          if (func) {
            var count = 0,
              object = { a: 1 }
