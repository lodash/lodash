...
describe('find methods', function () {
  lodashStable.each(
    ...
    function (methodName) {
      ...
      it(
        '`_.' + methodName + '` should work with `_.property` shorthands',
        function () {
      ...
      it(
        ...
        function () {
          var emptyValues = lodashStable.endsWith(methodName, 'Index')
              ? lodashStable.reject(empties, lodashStable.isPlainObject)
              : empties,
            expecting = lodashStable.map(
              emptyValues,
      ...
      it(
        '`_.' +
          methodName +
          '` should not execute immediately when explicitly chaining',
        function () {
    ...
    function () {
      each(
        ...
        function (methodName) {
          ...
          it(
            '`_.' +
              methodName +
              '` should provide correct `predicate` arguments for arrays',
            function () {
              var args,
                array = ['a']
      ...
      each(
        ['find', 'findKey', 'findLast', 'findLastKey'],
        function (methodName) {
          var func = _[methodName]
          ...
          it(
            '`_.' +
              methodName +
              '` should provide correct `predicate` arguments for objects',
            function () {
              var args,
                object = { a: 1 }
