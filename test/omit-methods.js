...
describe('omit methods', function () {
  lodashStable.each(['omit', 'omitBy'], function (methodName) {
    ...
    it(
      '`_.' +
        methodName +
        '` should create an object with omitted string keyed properties',
      function () {
        assert.deepStrictEqual(func(object, resolve(object, 'a')), {
          b: 2,
          c: 3,
          d: 4
        })
        assert.deepStrictEqual(
          func(object, resolve(object, ['a', 'c'])),
          expected
        )
      }
    )
    ...
    it(
      ...
      function () {
        ...
        assert.deepStrictEqual(
          func(new Foo(), resolve(object, ['a', 'c'])),
          expected
        )
      }
    )
    ...
    it(
      '`_.' + methodName + '` should work with an array `object`',
      function () {
        var array = [1, 2, 3]
        assert.deepStrictEqual(func(array, resolve(array, ['0', '2'])), {
          1: 2
        })
      }
    )
