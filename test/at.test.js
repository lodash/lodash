...
describe('at', function () {
  ...
  it('should work with non-index keys on array values', function () {
    var values = lodashStable
      .reject(empties, function (value) {
        return value === 0 || lodashStable.isArray(value)
      })
      .concat(-1, 1.1)

    var array = lodashStable.transform(
      values,
  ...
  it('should work with a falsey `object` when keys are given', function () {
    var expected = lodashStable.map(
      falsey,
      lodashStable.constant(Array(4).fill(undefined))
    )
  ...
  it('should support shortcut fusion', function () {
    ...
    lodashStable.each(
      [lastIndex, lastIndex + '', LARGE_ARRAY_SIZE, []],
      function (n, index) {
        count = 0
        ...
        expected =
          index == 3 ? [] : [index == 2 ? undefined : square(lastIndex)]
