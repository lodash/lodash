...
describe('flow methods', function () {
  lodashStable.each(['flow', 'flowRight'], function (methodName) {
    ...
    it(
      '`_.' +
        methodName +
        '` should supply each function with the return value of the previous',
      function () {
        var fixed = function (n) {
            return n.toFixed(1)
          },
          combined = isFlow
            ? func(add, square, fixed)
            : func(fixed, square, add)
    ...
    it(
      ...
      function () {
        ...
        var combined = isFlow ? func(head, curried) : func(curried, head)
    ...
    it(
      '`_.' +
        methodName +
        '` should work with curried functions with placeholders',
      function () {
        var curried = lodashStable.curry(lodashStable.ary(map, 2), 2),
          getProp = curried(curried.placeholder, (value) => value.a),
          objects = [{ a: 1 }, { a: 2 }, { a: 1 }]

        var combined = isFlow ? func(getProp, uniq) : func(uniq, getProp)
