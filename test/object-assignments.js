...
describe('object assignments', function () {
  lodashStable.each(
    ['assign', 'assignIn', 'defaults', 'defaultsDeep', 'merge'],
    function (methodName) {
      ...
      it(
        '`_.' + methodName + '` should not skip a trailing function source',
        function () {
          function fn() {}
          fn.b = 2
      ...
      it(
        ...
        function () {
          try {
            assert.deepStrictEqual(func({ a: 1 }, undefined, { b: 2 }, null), {
              a: 1,
              b: 2
            })
          } catch (e) {
      ...
      it(
        '`_.' +
          methodName +
          '` should create an object when `object` is nullish',
        function () {
          var source = { a: 1 },
      ...
      it(
        ...
        function () {
          ...
          assert.deepStrictEqual(
            lodashStable.reduce(array, func, { a: 0 }),
            expected
          )
          assert.deepStrictEqual(
            lodashStable.reduce(fn, func, { a: 0 }),
            expected
          )
        }
      )
  ...
  lodashStable.each(['assign', 'assignIn', 'merge'], function (methodName) {
    ...
    it(
      '`_.' + methodName + '` should not treat `object` as `source`',
      function () {
        function Foo() {}
        Foo.prototype.a = 1
  ...
  lodashStable.each(
    ...
    function (methodName) {
      ...
      it(
        '`_.' +
          methodName +
          '` should not assign values that are the same as their destinations',
        function () {
  ...
  lodashStable.each(
    ['assignWith', 'assignInWith', 'mergeWith'],
    function (methodName) {
      var func = _[methodName],
      ...
      it(
        '`_.' +
          methodName +
          '` should not treat the second argument as a `customizer` callback',
        function () {
          function callback() {}
          callback.b = 2
