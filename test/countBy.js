...
describe('countBy', function() {
  ...
  it('should transform keys by `iteratee`', function() {
    let actual = countBy(array, Math.floor);
  ...
  it('should work with an object for `collection`', function() {
    let actual = countBy({ 'a': 6.1, 'b': 4.2, 'c': 6.3 }, Math.floor);
    assert.deepStrictEqual(actual, { '4': 1, '6': 2 });
  });

  it('should work in a lazy sequence', function() {
    let array = lodashStable.range(LARGE_ARRAY_SIZE).concat(
    ...
    let actual = _(array).countBy().map(square).filter(isEven).take().value();
